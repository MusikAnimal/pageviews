...
module.exports = {
  'Changing chart type updates the view and sets localStorage': client => {
    ...
    // set autoChartType functionality to false so chart type changes are remembered
    client.click('.js-test-settings');
    client.expect.element('#settings-modal').to.be.visible.after(1000);
    client.click('.js-test-remember-chart');
    client.click('.save-settings-btn');
    client.execute('return localStorage[\'pageviews-settings-rememberChart\']', [], response => {
      client.expect(response.value).to.equal('true');
    });
