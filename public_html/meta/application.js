...
var MetaViews = function (_mix$with) {
  ...
  _createClass(MetaViews, [{
    ...
    value: function processInput(force) {
      ...
      var entities = $(this.config.select2Input).select2('val') || [];
      ...
      xhrData.entities.forEach(function (tool, index) {
        var url = '/pageviews/meta/api.php?app=' + tool + '&start=' + startDate.format('YYYY-MM-DD') + ('&end=' + endDate.format('YYYY-MM-DD') + '&type=timeline');
        ...
        promise.done(function (successData) {
          ...
          if (!xhrData.labels.length) {
            xhrData.labels = successData.map(function (elem) {
              return moment(elem.date, 'YYYY-MM-DD').format(_this3.dateFormat);
            });
          }
        }).fail(function (data) {
          _this3.writeMessage('<a href=\'/' + tool.escape() + '\'>' + tool.escape() + '</a> - ' + $.i18n('api-error-no-data'));
    ...
    value: function updateTable() {
      ...
      $('.output-list .table-view--title a').off('click').on('click', function (e) {
        _this4.showProjectUsage($(e.target).text());
      });
    ...
    key: 'showSingleAppLegend',
    value: function showSingleAppLegend() {
      var app = this.outputData[0];
    ...
    value: function showProjectUsage(app) {
      var _this5 = this;
      ...
      $.ajax({
        url: '/pageviews/meta/api.php?app=' + app + '&type=projects',
        dataType: 'json'
      }).done(function (data) {
...
},{"../shared/chart_helpers":3,"../shared/pv":6,"./config":1}],3:[function(require,module,exports){
'use strict';
