...
},{"./zoom_plugin":8}],2:[function(require,module,exports){
'use strict';
...
},{"./core_extensions":2,"./polyfills":4,"./pv_config":6,"./site_map":7}],6:[function(require,module,exports){
'use strict';
...
var UserViews = function (_mix$with) {
  _inherits(UserViews, _mix$with);
  ...
  function UserViews() {
    _classCallCheck(this, UserViews);

    var _this = _possibleConstructorReturn(this, (UserViews.__proto__ || Object.getPrototypeOf(UserViews)).call(this, config));
  ...
  _createClass(UserViews, [{
    ...
    value: function setupListeners() {
      ...
      _get(UserViews.prototype.__proto__ || Object.getPrototypeOf(UserViews.prototype), 'setupListeners', this).call(this);
    ...
    value: function setupNamespaceSelector() {
      ...
      var namespace = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 0;

      var dfd = $.Deferred();

      this.fetchSiteInfo(this.project).then(function (data) {
        $('#namespace_input').html('<option value=\'all\'>' + $.i18n('all') + '</option>');

        var namespaces = data[_this3.project].namespaces;
        for (var ns in namespaces) {
          if (ns < 0) continue;
          var nsTitle = namespaces[ns]['*'] || $.i18n('main');
    ...
    value: function buildMotherDataset(label, link, datasets) {
      ...
      totalTitles = [],
          totalSize = 0,
          oldest = void 0,
          newest = void 0; // moment object for the dates of the oldest and newest pages
      ...
      datasets.forEach(function (dataset, index) {
        ...
        var dateCreated = moment(dataset.timestamp.substr(0, 8), 'YYYYMMDD');
      ...
      var datespan = oldest && newest ? Math.abs(oldest.diff(newest, 'days')) : null;
     ...
     * @return {string} HTML markup
     */
    value: function getUserLink(user) {
    ...
    value: function pushParams() {
      ...
      var escapedPageName = $(this.config.sourceInput).val().score().replace(/[&%?]/g, escape);
    ...
    key: 'checkEditCount',
    value: function checkEditCount(user) {
      var _this7 = this;
      ...
      $.ajax({
        url: 'https://' + this.project + '.org/w/api.php?action=query&list=users' + ('&ususers=' + user + '&usprop=editcount&format=json'),
        dataType: 'jsonp'
      }).then(function (data) {
        var editCount = data.query.users[0].editcount;
        if (editCount > 50000) {
          var userLink = _this7.getUserLink(user);
          _this7.toastInfo($.i18n('userviews-edit-count-warning', userLink, editCount));
        }
      });
    }
    ...
    value: function getPagesCreated() {
      ...
      $.ajax({
        ...
        var pages = data.map(function (page) {
          var ns = _this8.siteInfo[_this8.project].namespaces[page.namespace]['*'],
              title = ns === '' ? page.title : ns + ':' + page.title;
          return Object.assign(page, { title: title });
        });
        if (pages.length >= _this8.config.apiLimit) {
          _this8.toastWarn($.i18n('userviews-oversized-set', _this8.getUserLink(username), _this8.config.apiLimit, _this8.config.apiLimit));
        }
    ...
    value: function getPageViewsData(pages) {
      ...
      var makeRequest = function makeRequest(pageObj) {
        var pageTitle = pageObj.title,
            uriEncodedPageName = encodeURIComponent(pageTitle);

        var url = 'https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/' + _this9.project + ('/' + $(_this9.config.platformSelector).val() + '/' + $(_this9.config.agentSelector).val() + '/' + uriEncodedPageName + '/daily') + ('/' + startDate.format(_this9.config.timestampFormat) + '/' + endDate.format(_this9.config.timestampFormat));
        ...
        promise.done(function (pvData) {
          ...
          /** first detect if this was a Cassandra backend error, and if so, schedule a re-try */
          var cassandraError = errorData.responseJSON.title === 'Error in Cassandra table storage backend',
              failedPageLink = _this9.getPageLink(pageTitle, _this9.project + '.org');
          ...
          if (cassandraError) {
            ...
            if (failureRetries[pageTitle] < 3) {
              totalRequestCount++;
              return _this9.rateLimit(makeRequest, _this9.config.apiThrottle, _this9)(pageTitle);
            }

            /** retries exceeded */
            failedPages.push(failedPageLink);
          } else {
            _this9.writeMessage(failedPageLink + ': ' + $.i18n('api-error', 'Pageviews API') + ' - ' + errorData.responseJSON.title);
          }

          // unless it was a 404, don't cache this series of requests
          if (errorData.status !== 404) hadFailure = true;
        }).always(function () {
          _this9.updateProgressBar(++count, totalRequestCount);

          if (count === totalRequestCount) {
            if (failedPages.length) {
              _this9.writeMessage($.i18n('api-error-timeout', '<ul>' + failedPages.map(function (failedPage) {
    ...
    value: function getInterwikiData(dbName, pageName) {
      ...
      $.getJSON(url).done(function (data) {
        if (data.error) {
          ...
          return dfd.reject('<a target=\'_blank\' href=\'' + _this10.getPageURL(pageName).escape() + '\'>' + pageName.descore().escape() + '</a> - ' + $.i18n('api-error-no-data'));
        }

        var key = Object.keys(data.entities)[0],
            sitelinks = data.entities[key].sitelinks,
            filteredLinks = {},
            matchRegex = new RegExp('^https://[\\w-]+\\.' + _this10.baseProject + '\\.org');
    ...
    value: function popParams() {
      ...
      if (params.user) {
        ...
        this.setupNamespaceSelector(params.namespace).then(function () {
    ...
    value: function processInput() {
      ...
      var readyForRendering = function readyForRendering() {
        $('.output-title').html(_this12.outputData.link);
      ...
      if (this.isRequestCached()) {
        ...
        return setTimeout(function () {
          _this12.outputData = simpleStorage.get(_this12.getCacheKey());
          readyForRendering();
        }, 500);
      }
      ...
      this.elapsedTimer = setInterval(function () {
        $('.elapsed-timer').text($.i18n('elapsed-time', ++count));
      }, 1000);
      ...
      this.getPagesCreated(user).done(function (pagesCreated) {
        if (!pagesCreated.length) {
          _this12.toastInfo($.i18n('select2-no-results'));
          return _this12.setState('initial');
        }
        _this12.getPageViewsData(pagesCreated).done(function (pageViewsData) {
          $('.progress-bar').css('width', '100%');
          $('.progress-counter').text($.i18n('building-dataset'));
          var userLink = _this12.getUserLink(user);
          setTimeout(function () {
            _this12.buildMotherDataset(user, userLink, pageViewsData);
            readyForRendering();
          }, 250);
        });
      }).fail(function (error) {
        ...
        if (typeof error === 'string') {
          ...
          _this12.toastError($.i18n('api-error-unknown', 'Page Creation API'));
        }
      }).always(function () {
        $('.elapsed-timer').text('');
        window.clearInterval(_this12.elapsedTimer);
        _this12.elapsedTimer = null;
      });
    }
    ...
    value: function validateProject() {
      ...
      if (_get(UserViews.prototype.__proto__ || Object.getPrototypeOf(UserViews.prototype), 'validateProject', this).call(this)) {
    ...
    value: function exportCSV() {
      ...
      this.outputData.listData.forEach(function (page) {
        var pageName = '"' + page.label.descore().replace(/"/g, '""') + '"',
          ...
          return _this13.config.badges[badge].name.replace(/"/g, '""');
        }) + '"';
...
},{"../shared/chart_helpers":1,"../shared/list_helpers":3,"../shared/pv":5,"../shared/site_map":7,"./config":9}]},{},[10]);
