...
var PageViews = function (_mix$with) {
  ...
  _createClass(PageViews, [{
    ...
    value: function setupListeners() {
      ...
      this.$platformSelector.add(this.$agentSelector).add(this.$redirectsCheckbox).on('change', this.processInput.bind(this));
    }
    ...
    key: 'consolidateRedirectData',
    value: function consolidateRedirectData(redirectsData, xhrData) {
      var _this6 = this;

      var targetTitles = Object.keys(redirectsData);
      ...
      var dateHeadings = this.getDateHeadings(false),
          dateFormat = this.isMonthly() ? 'YYYY-MM' : 'YYYY-MM-DD';

      Object.keys(redirectsData).forEach(function (target, targetIndex) {
        // Collect all the titles of the redirects for this `target` page.
        var allTitles = redirectsData[target].map(function (rd) {
          return rd.title;
        });

        allTitles.forEach(function (title) {
          // Find index within `xhrData.entities`.
          var redirIndex = xhrData.entities.indexOf(title);
          ...
          var redirDataset = xhrData.datasets[redirIndex];

          // Loop through each item in the timeseries, summing redirect pageviews as if they were all for the target page.
          redirDataset.map(function (elem) {
            var date = moment(elem.timestamp, _this6.config.timestampFormat).format(dateFormat);
            var index = dateHeadings.indexOf(date);
            var newViews = newXhrData.datasets[targetIndex] && newXhrData.datasets[targetIndex][index] ? newXhrData.datasets[targetIndex][index].views : 0;
    ...
    value: function processInput(force, removedPage) {
      ...
      var getPageViewsAndAssessments = function getPageViewsAndAssessments(entities) {
        _this7.getRedirects(entities).done(function (redirectsData) {
          // Fetch all the redirects and concat into `entities`.
          Object.keys(redirectsData).forEach(function (target) {
            entities = entities.concat(redirectsData[target].map(function (rd) {
              return rd.title;
            }));
          });
          _this7.getPageViewsData(entities.unique()).done(function (xhrData) {
            _this7.updateChart(_this7.consolidateRedirectData(redirectsData, xhrData));
          });
        });
      };
  ...
  return function (_superclass) {
    ...
    _createClass(_class, [{
      ...
      value: function getPageViewsData(entities) {
         ...
         */
        var targetEntities = this.getEntities(true);
        var shouldShowErrors = function shouldShowErrors(entity) {
            ...
            if (cassandraError) {
              ...
              if (_this8.app === 'pageviews' && errorData.status === 404) {
                ...
                $.ajax({
                  ...
                  if (dateCreated && moment(dateCreated).isAfter(_this8.maxDate) && shouldShowErrors(entity)) {
...
var Pv = function (_PvConfig) {
  ...
  function Pv(appConfig) {
    ...
    ['alwaysRedirects', 'localizeDateFormat', 'numericalFormatting', 'bezierCurve', 'autocomplete', 'autoLogDetection', 'beginAtZero', 'rememberChart'].forEach(function (setting) {
  ...
  _createClass(Pv, [{
     ...
     * @return {Boolean}
     */
    value: function includeRedirects() {
    ...
    value: function getEntities() {
      var score = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : false;
