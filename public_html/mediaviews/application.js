...
var MediaViews = function (_mix$with) {
  ...
  _createClass(MediaViews, [{
    ...
    value: function popParams() {
      ...
      var noFiles = !params.files || !params.files.length || params.files.length === 1 && !params.files[0];
       ...
       */
      this.getFileInfo(params.files.unique()).then(function (fileInfo) {
        var normalizedFileNames = Object.keys(fileInfo.entities);

        // all given pages were invalid, reset view without clearing the error message
        if (!normalizedFileNames.length) return _this2.resetView(false, false);

        _this2.setSelect2Defaults(_this2.underscorePageNames(normalizedFileNames));
      });
    }
    ...
    value: function getFileInfo(files) {
      ...
      $.ajax({
        url: 'https://' + this.project + '.org/w/api.php?action=query&prop=imageinfo&' + ('iiprop=mediatype|size|timestamp|url&formatversion=2&format=json&titles=' + files.join('|')),
        dataType: 'jsonp'
      }).done(function (data) {
        ...
        var fileData = _this3.entityInfo.entities || {};
        files.forEach(function (file) {
          ...
          if (fileInfo.missing) {
            _this3.writeMessage(_this3.getFileLink(file) + ': ' + $.i18n('api-error-no-data'));
            return;
          }
        ...
        _this3.entityInfo = { entities: fileData };

        return dfd.resolve(_this3.entityInfo);
      }).fail(function () {
    ...
    value: function pushParams() {
      _get(MediaViews.prototype.__proto__ || Object.getPrototypeOf(MediaViews.prototype), 'pushParams', this).call(this, 'files');
    }
    ...
    value: function setupSelect2() {
        ...
        ajax: {
          ...
          processResults: function processResults(data) {
            ...
            if (query.allimages.length) {
              var titles = query.allimages.map(function (elem) {
                return elem.name;
              });

              results = titles.map(function (filename) {
                return {
                  id: filename.score(),
                  text: filename
                };
      ...
      _get(MediaViews.prototype.__proto__ || Object.getPrototypeOf(MediaViews.prototype), 'setupSelect2', this).call(this, params);
    }
    ...
    value: function setupListeners() {
      ...
      $('#date-type-select').on('change', function (e) {
        ...
        if (e.target.value === 'monthly') {
          ...
          _this4.daterangepicker.setStartDate(_this4.monthStartDatepicker.getDate());
          _this4.daterangepicker.setEndDate(moment(_this4.monthEndDatepicker.getDate()).endOf('month'));
        } else {
          _this4.processInput();
        }
      });
      $('.sort-link').on('click', function (e) {
        ...
        _this4.direction = _this4.sort === sortType ? -_this4.direction : 1;
        _this4.sort = sortType;
        _this4.updateTable();
      });
      $('.clear-pages').on('click', function () {
        _this4.resetView(true);
        _this4.focusSelect2();
      });
    }
    ...
    value: function processInput(force, removedFile) {
      ...
      var files = this.beforeProcessInput(force);
      if (!files) {
        return;
      }
      ...
      if (removedFile) {
        ...
        this.outputData = this.outputData.map(function (entity) {
          return Object.assign({}, entity, _this5.config.chartConfig[_this5.chartType].dataset(entity.color));
        });
        delete this.entityInfo.entities[removedFile];
        this.updateChart();
      } else {
        this.getFileInfo(files).then(function () {
          _this5.getRequestCounts(files).done(function (xhrData) {
            _this5.updateChart(xhrData);
          });
        });
      }
    }
    ...
    key: 'getMvApiUrl',
    value: function getMvApiUrl(file, startDate, endDate) {
      var granularity = $('#date-type-select').val() || 'daily',
          agent = this.$agentSelector.val() || this.config.defaults.agent;
      return 'https://wikimedia.org/api/rest_v1/metrics/mediarequests/per-file/' + $('#referer-select').val() + ('/' + agent + '/' + encodeURIComponent(file) + '/' + granularity + '/') + (startDate.format(this.config.timestampFormat) + '/' + endDate.format(this.config.timestampFormat));
    }
    ...
    key: 'getRequestCounts',
    value: function getRequestCounts(files) {
      var _this6 = this;
      ...
      var makeRequest = function makeRequest(file, index) {
        var url = _this6.getMvApiUrl(_this6.entityInfo.entities[file.descore()].path, startDate, endDate),
        ...
        promise.done(function (successData) {
          try {
            ...
            if (successData.items && !xhrData.labels.length) {
              xhrData.labels = successData.items.map(function (elem) {
                return moment(elem.date, _this6.config.timestampFormat).format(_this6.dateFormat);
              });
            }
          } catch (err) {
          ...
          var link = _this6.getFileLink(file);

          xhrData.errors.push(link + ': ' + $.i18n('api-error', 'Media requests API') + ' - ' + errorData.responseJSON.title);
        }).always(function () {
          if (++count === totalRequestCount) {
            dfd.resolve(xhrData);

            if (failedFiles.length) {
              _this6.writeMessage($.i18n('api-error-timeout', '<ul>' + failedFiles.map(function (failedFile) {
                return '<li>' + _this6.getFileLink(failedFile) + '</li>';
              }).join('') + '</ul>'));
            }
          }
        });
    ...
    value: function updateTable() {
      ...
      ['duration', 'size'].forEach(function (type) {
      ...
      $('.table-view--duration').toggle(datasets.some(function (ds) {
        return !!ds.duration;
      }));
    ...
    value: function getFileLink(file) {
      return _get(MediaViews.prototype.__proto__ || Object.getPrototypeOf(MediaViews.prototype), 'getPageLink', this).call(this, this.addNamespace(file), this.project, file.replace(/^File:/, ''));
    }
    ...
    key: 'stripNamespace',
    value: function stripNamespace(entity) {
      var readd = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : false;
    ...
    key: 'addNamespace',
    value: function addNamespace(entity) {
    ...
    value: function validateProject() {
      if (_get(MediaViews.prototype.__proto__ || Object.getPrototypeOf(MediaViews.prototype), 'validateProject', this).call(this)) {
        this.resetView(true);
        this.focusSelect2();
      }
    }
...
var templates = {
  chartLegend: function chartLegend(scope) {
    var dataList = function dataList(entity) {
      ...
      var infoHash = _defineProperty({}, $.i18n('requests'), (_$$i18n = {}, _defineProperty(_$$i18n, $.i18n('requests'), scope.formatNumber(entity.sum)), _defineProperty(_$$i18n, $.i18n($('#date-type-select').val() + '-average'), scope.formatNumber(entity.average)), _$$i18n));
