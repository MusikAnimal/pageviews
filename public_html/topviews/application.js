...
var TopViews = function (_Pv) {
  ...
  _createClass(TopViews, [{
    ...
    value: function drawData() {
      ...
      this.getPageDetails(pageNames).then(function () {
        count = 0;

        pageIndexes.forEach(function (pageIndex) {
          var item = _this2.pageData[pageIndex];

          $('.chart-container').append('<article class=\'topview-entry clearfix\'>\n             <span class=\'topview-entry--remove glyphicon glyphicon-remove\' data-article-id=' + (pageIndex - 1) + ' aria-hidden=\'true\'></span>\n             <span class=\'topview-entry--rank\'>' + ++count + '</span>\n             <span class=\'topview-entry--image\' style=\'background-image:url(' + item.thumbnail + ')\' />\n             <span class=\'topview-entry--details\'>\n               <a class=\'topview-entry--label\' href="' + _this2.getPageURL(item.article) + '" target=\'_blank\'>' + item.article + '</a>\n               <a class=\'topview-entry--views pull-right\' target=\'_blank\' href=\'' + _this2.getPageviewsURL(item.article) + '\'>' + _this2.formatNumber(item.views) + '</a>\n               <div class=\'topview-entry--intro clearfix\'>\n                 ' + (item.intro || '') + '\n               </div>\n             </span>\n           </article>');
    ...
    key: 'getPageDetails',
    value: function getPageDetails(pageNames) {
      var _this5 = this;

      var dfd = $.Deferred();
      var url = 'https://' + this.project + '.org/w/api.php?action=query&titles=' + pageNames.join('|') + '&' + 'prop=pageimages|extracts&pithumbsize=500&exintro=1&exsentences=3&explaintext=1&' + 'exlimit=10&pilimit=10&format=json&formatversion=2';
      ...
      $.ajax({
        ...
        data.query.pages.forEach(function (page) {
          var index = _this5.pageNames.indexOf(page.title);
          var thumbnail = 'https://upload.wikimedia.org/wikipedia/commons/thumb/d/d6/Antu_wikipedia.svg/150px-Antu_wikipedia.svg.png';
          ...
          Object.assign(_this5.pageData[index], {
            thumbnail: thumbnail,
            intro: page.extract
          });
        });
    ...
    value: function searchTopviews() {
      ...
      matchedData.forEach(function (item) {
        ...
        $('.chart-container').append('<div class=\'topview-entry clearfix\' style=\'background:linear-gradient(' + direction + ', #EEE ' + width + '%, transparent ' + width + '%)\'>\n         <span class=\'topview-entry--remove glyphicon glyphicon-remove\' data-article-id=' + item.index + ' aria-hidden=\'true\'></span>\n         <span class=\'topview-entry--rank\'>' + item.rank + '</span>\n         <a target=\'_blank\' class=\'topview-entry--label\' href="' + _this7.getPageURL(item.article) + '" target="_blank">' + item.article + '</a>\n         <span class=\'topview-entry--leader\'></span>\n         <a target=\'_blank\' class=\'topview-entry--views\' href=\'' + _this7.getPageviewsURL(item.article) + '\'>' + _this7.formatNumber(item.views) + '</a></div>');
      });
    ...
    value: function setSelect2Defaults(pages) {
      ...
      pages = pages.map(function (page) {
        ...
        $('<option>' + escapedText + '</option>').appendTo(_this9.config.select2Input);
        return page;
      });
    ...
    value: function initData() {
      ...
      $.ajax({
        ...
        _this11.pageData = data.items[0].articles.map(function (page) {
          page.article = page.article.descore();
          return page;
        });

        /** build the pageNames array for Select2 */
        _this11.pageNames = _this11.pageData.map(function (page) {
          return page.article;
        });

        if ($('.mainspace-only-option').is(':checked')) {
          _this11.filterOutNamespace(_this11.pageNames).done(function (pageNames) {
            _this11.pageNames = pageNames;
            _this11.pageData = _this11.pageData.filter(function (page) {
              return pageNames.includes(page.article);
            });
            return dfd.resolve(_this11.pageData);
          });
        } else {
          return dfd.resolve(_this11.pageData);
        }
        ...
        _this11.resetView();
        _this11.writeMessage($.i18n('api-error', 'Pageviews API') + ' - ' + errorData.responseJSON.title);
        return dfd.reject();
      });
    ...
    value: function filterOutNamespace(pages) {
      ...
      var doFiltering = function doFiltering(entries, unacceptableNamespaces) {
        return entries.filter(function (entry) {
          ...
          if (restrictedNamespace === 0 && entry === _this12.getSiteInfo(_this12.project).general.mainpage) {
            return false;
          }
      ...
      this.fetchSiteInfo(this.project).done(function () {
        ...
        for (var ns in _this12.getSiteInfo(_this12.project).namespaces) {
          unacceptableNamespaces.push(_this12.getSiteInfo(_this12.project).namespaces[ns]['*']);
        }
        ...
        _this12.excludes = doFiltering(_this12.excludes, unacceptableNamespaces);

        dfd.resolve(pages);
      }).fail(function () {
        _this12.writeMessage('' + $.i18n('api-error', 'Siteinfo API'));
        dfd.resolve(pages);
      });
