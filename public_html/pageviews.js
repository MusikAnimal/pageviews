...
var PageViews = function (_mix$with) {
  ...
  _createClass(PageViews, [{
    ...
    value: function processInput(force) {
       ...
       */
      xhrData.entities.forEach(function (article, index) {
        var uriEncodedArticle = encodeURIComponent(article);

        /** @type {String} Url to query the API. */
        var url = 'https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/' + _this6.project + ('/' + $(_this6.config.platformSelector).val() + '/' + $('#agent-select').val() + '/' + uriEncodedArticle + '/daily') + ('/' + startDate.format(_this6.config.timestampFormat) + '/' + endDate.format(_this6.config.timestampFormat));
        var promise = $.ajax({
          url: url,
          dataType: 'json'
        });
        ...
        promise.success(function (successData) {
          try {
            ...
            if (_this6.config.linearCharts.includes(_this6.chartType)) {
              xhrData.datasets.push(_this6.getLinearData(successData, article, index));
            } else {
              xhrData.datasets.push(_this6.getCircularData(successData, article, index));
            }
