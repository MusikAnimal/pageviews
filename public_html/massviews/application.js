...
var MassViews = function (_mix$with) {
  ...
  _createClass(MassViews, [{
    ...
    value: function updateSourceInput(node) {
      ...
      this.$sourceInput.prop('type', this.config.sources[source].type).prop('placeholder', this.config.sources[source].placeholder).val('');
    ...
    value: function getPageViewsData(pages, project) {
      ...
      var makeRequest = function makeRequest(page) {
        ...
        var url = 'https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/' + queryProject + ('/' + _this5.$platformSelector.val() + '/' + _this5.$agentSelector.val() + '/' + uriEncodedPageName + '/daily') + ('/' + startDate.format(_this5.config.timestampFormat) + '/' + endDate.format(_this5.config.timestampFormat));
        ...
        promise.done(function (pvData) {
          ...
          if (cassandraError) {
            ...
            _this5.writeMessage(_this5.getPageLink(page, queryProject) + ': ' + $.i18n('api-error', 'Pageviews API') + ' - ' + errorMessage);
          }

          // Don't cache this series of requests.
          _this5.hadFailure = true;
        }).always(function () {
    ...
    value: function getPagePile(id) {
      ...
      var dfd = $.Deferred();
      var url = 'https://tools.wmflabs.org/pagepile/api.php?id=' + id + '&action=get_data' + ('&format=json&metadata=1&limit=' + this.config.apiLimit);
      ...
      $.ajax({
        ...
        if (data.pages_total > _this7.config.apiLimit) {
          _this7.writeMessage($.i18n('massviews-oversized-set', _this7.getPileLink(id), _this7.formatNumber(data.pages_total), _this7.config.apiLimit, data.pages_total));
    ...
    value: function getWikiPageFromURL(url) {
      ...
      if (matches) {
        return [matches[1].replace(/^www\./, ''), matches[2]];
      } else {
        return [null, null];
      }
    }
    ...
    value: function processPagePile(cb) {
      ...
      var pileId = this.$sourceInput.val();
    ...
    value: function processHashtag(cb) {
      ...
      var hashtag = this.$sourceInput.val().replace(/^#/, ''),
      ...
      $.get('//tools.wmflabs.org/hashtags/csv/' + hashtag + '?limit=5000').done(function (data) {
        ...
        _this12.getPageURLsFromHashtagCSV(csvData).done(function (pageURLs) {
          ...
          if (size > _this12.config.apiLimit) {
            _this12.writeMessage($.i18n('massviews-oversized-set', hashTagLink, _this12.formatNumber(size), _this12.config.apiLimit, size));
    ...
    value: function processSubpages(project, targetPage, cb) {
      ...
      (_$2 = $).when.apply(_$2, promises).done(function (data, data2) {
        ...
        if (size > _this14.config.apiLimit) {
          _this14.writeMessage($.i18n('massviews-oversized-set', pageLink, _this14.formatNumber(size), _this14.config.apiLimit, size));
    ...
    value: function processQuarry(cb) {
      ...
      $.getJSON(url).done(function (data) {
        ...
        if (titles.length > _this17.config.apiLimit) {
          _this17.writeMessage($.i18n('massviews-oversized-set', quarryLink, _this17.formatNumber(titles.length), _this17.config.apiLimit, titles.length));
    ...
    value: function processSearch(cb) {
      ...
      var srsearch = this.$sourceInput.val();
    ...
    value: function processInput() {
      ...
      var readyForRendering = function readyForRendering() {
        ...
        $('.output-params').html(_this20.$dateRangeSelector.val());
      ...
      var _getWikiPageFromURL3 = this.getWikiPageFromURL(this.$sourceInput.val()),
          _getWikiPageFromURL4 = _slicedToArray(_getWikiPageFromURL3, 2),
          project = _getWikiPageFromURL4[0],
          target = _getWikiPageFromURL4[1];
  ...
  return function (_superclass) {
    ...
    _createClass(_class, [{
      ...
      value: function setColorsAndLogValues(outputData) {
        ...
        select2Values = ($(this.config.select2Input).select2('val') || []).map(function (title) {
