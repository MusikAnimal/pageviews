  ...
  _createClass(MassViews, [{
    ...
    value: function renderData() {
      ...
      _get(MassViews.prototype.__proto__ || Object.getPrototypeOf(MassViews.prototype), 'renderData', this).call(this, function (sortedDatasets) {
        ...
        var glamorousCols = '';
        if (_this4.source === 'glamorous') {
          glamorousCols = '\n          <th>' + _this4.formatNumber(_this4.outputData.totalSources) + ' sources</th>\n          <th>' + $.i18n('num-projects', _this4.formatNumber(_this4.outputData.totalProjects)) + '</th>\n        ';
        }
        ...
        sortedDatasets.forEach(function (item, index) {
          ...
          if (_this4.source === 'glamorous') {
            var pageLink = '<a href=\'https://commons.wikimedia.org/wiki/' + item.globalPage + '\' target=\'_blank\'>\n            ' + item.globalPage.replace(/^File:/, '') + '\n          </a>';

            glamorousCols = '\n            <td class=\'glamorous-cols\'>' + pageLink + '</td>\n            <td class=\'glamorous-cols\'>' + _this4.getSiteLink(item.project.replace(/\.org$/, '')) + '</td>\n          ';
          }
        ...
        $('.list-view').addClass('data-source-' + _this4.source);
      });
    }
    ...
    value: function getPageViewsData(pages, project) {
      ...
      var makeRequest = function makeRequest(page) {
        ...
        promise.done(function (pvData) {
          ...
          if (showProgress) _this5.updateProgressBar(count, totalRequestCount);
    ...
    value: function buildMotherDataset(label, link, datasets, showDatesWithoutData) {
      var _this6 = this;
    ...
    key: 'processCommonsCat',
    value: function processCommonsCat() {
      var _this11 = this;

      var category = $(this.config.sourceInput).val();
      ...
      var categoryLink = this.getPageLink('Category:' + category, 'commons.wikimedia.org');

      $('.progress-counter').text($.i18n('fetching-data', 'Category API'));
      this.massApi(requestData, 'commons.wikimedia.org', 'cmcontinue', 'categorymembers').done(function (data) {
        if (data.error) {
        ...
        _this11.getGlobalUsage(pageNames).done(function (globalData) {
          var promises = {},
              totalRequestCount = 0;

          // build sequence of getPageViewsData promises
          for (var globalPage in globalData) {
            for (var wiki in globalData[globalPage]) {
              var titles = globalData[globalPage][wiki];
          ...
          var globalPageViewsData = {},
              progressCount = 0;

          var buildDataset = function buildDataset() {
            $('.progress-bar').css('width', '100%');
            $('.progress-counter').text($.i18n('building-dataset'));

            var motherDataset = void 0;

            setTimeout(function () {
              var _loop = function _loop(_globalPage) {
                var datasets = globalPageViewsData[_globalPage];
                var childDataset = _this11.buildMotherDataset('Category:' + category, categoryLink, datasets);

                childDataset.listData = childDataset.listData.map(function (entry) {
                  entry.globalPage = _globalPage;
                  return entry;
                });
                ...
                if (motherDataset) {
                  (function () {
                    ...
                    motherDataset.datesWithoutData = motherDataset.datesWithoutData.concat(childDataset.datesWithoutData).unique();

                    var totalViewsSet = motherDataset.datasets[0];
                    totalViewsSet.data = childDataset.datasets[0].data.map(function (num, i) {
                      return num + totalViewsSet.data[i];
                    });
                    totalViewsSet.sum = totalViewsSet.data.reduce(function (a, b) {
                      return (a || 0) + (b || 0);
                    });
              ...
              for (var _globalPage in globalPageViewsData) {
                _loop(_globalPage);
              }

              motherDataset.totalSources = Object.keys(globalPageViewsData).length;
              motherDataset.totalProjects = motherDataset.listData.map(function (entry) {
                return entry.project;
              }).unique().length;

              if (motherDataset.datesWithoutData.length) {
                var dateList = motherDataset.datesWithoutData.map(function (date) {
                  return moment(date).format(_this11.dateFormat);
                });
                _this11.writeMessage($.i18n('api-incomplete-data', dateList.sort().join(' &middot; '), dateList.length));
              }

              _this11.outputData = motherDataset;

              $('.output-title').html(_this11.outputData.link);
          ...
          _this11.updateProgressBar(0, totalRequestCount);

          var _loop2 = function _loop2(_globalPage2) {
            var makeRequest = function makeRequest(titles, project) {
              _this11.getPageViewsData(titles, project, false).done(function (pageViewsData) {
                globalPageViewsData[_globalPage2] = (globalPageViewsData[_globalPage2] || []).concat(pageViewsData);
                progressCount += titles.length;
                _this11.updateProgressBar(progressCount, totalRequestCount);

                if (progressCount === totalRequestCount) buildDataset();
              });
            };
            var requestFn = _this11.rateLimit(makeRequest, _this11.config.apiThrottle, _this11);

            promises[_globalPage2].forEach(function (params) {
              requestFn.apply(undefined, _toConsumableArray(params));
            });
          };

          for (var _globalPage2 in promises) {
            _loop2(_globalPage2);
          }
        });
      });
    ...
    key: 'getGlobalUsage',
    value: function getGlobalUsage(pages) {
      var dfd = $.Deferred();
      ...
      this.massApi(requestData, 'commons.wikimedia.org', 'gucontinue', 'pages').done(function (data) {
        data.pages.forEach(function (page) {
          if (page.globalusage) {
            globalUsage[page.title] = globalUsage[page.title] || {};

            page.globalusage.forEach(function (globalPage) {
    ...
    value: function processCategory(project, category, cb) {
      ...
      this.massApi(requestData, project, 'cmcontinue', 'categorymembers').done(function (data) {
        if (data.error) {
          return _this12.apiErrorReset('Category API', data.error.info);
        }
        ...
        namespaces = _this12.getSiteInfo(project) ? _this12.getSiteInfo(project).namespaces : undefined;
        var pages = data.categorymembers;
        ...
        if (size > _this12.config.apiLimit) {
          _this12.writeMessage($.i18n('massviews-oversized-set', categoryLink, _this12.formatNumber(size), _this12.config.apiLimit));

          pages = pages.slice(0, _this12.config.apiLimit);
        }

        var pageNames = _this12.mapCategoryPageNames(pages, namespaces);

        _this12.getPageViewsData(pageNames, project).done(function (pageViewsData) {
      ...
      $.get('//tools.wmflabs.org/hashtags/csv/' + hashtag + '?limit=5000').done(function (data) {
        ...
        if (csvData.length === 1) {
          return _this13.setState('initial', function () {
            _this13.writeMessage($.i18n('massviews-empty-set', hashTagLink));
          });
        }

        // collect necessary data from the other rows
        _this13.getPageURLsFromHashtagCSV(csvData).done(function (pageURLs) {
          var size = pageURLs.length;

          if (size > _this13.config.apiLimit) {
            _this13.writeMessage($.i18n('massviews-oversized-set', hashTagLink, _this13.formatNumber(size), _this13.config.apiLimit));

            pageURLs = pageURLs.slice(0, _this13.config.apiLimit);
          }

          _this13.getPageViewsData(pageURLs).done(function (pageViewsData) {
            cb(hashtag, hashTagLink, pageViewsData);
          });
        }).fail(function () {
          return _this13.apiErrorReset('Siteinfo API');
        });
      }).fail(function () {
        return _this13.apiErrorReset('Hashtag API');
      });
    }
    ...
    value: function getPageURLsFromHashtagCSV(csvData) {
      ...
      csvData.slice(1).forEach(function (entry) {
        ...
        _this14.fetchSiteInfo(project).done(function () {
          var nsName = _this14.getSiteInfo(project).namespaces[entry[namespaceIndex]]['*'];
    ...
    value: function processSubpages(project, targetPage, cb) {
      ...
      [namespace, inverseNamespace].forEach(function (apnamespace) {
        ...
        promises.push(_this15.massApi(params, project, 'apcontinue', 'allpages'));
      });
      ...
      (_$2 = $).when.apply(_$2, promises).done(function (data, data2) {
        ...
        if (errors.length) {
          _this15.setState('initial', function () {
            errors.forEach(function (error) {
              _this15.writeMessage($.i18n('api-error', 'Allpages API') + ': ' + error.error.info.escape());
            });
          });
          return false;
        }
        ...
        if (size > _this15.config.apiLimit) {
          _this15.writeMessage($.i18n('massviews-oversized-set', pageLink, _this15.formatNumber(size), _this15.config.apiLimit));

          pages = pages.slice(0, _this15.config.apiLimit);
        }
        ...
        _this15.getPageViewsData(pageNames, project).done(function (pageViewsData) {
        ...
        /** structured error comes back as a string, otherwise we don't know what happened */
        if (data && typeof data.error === 'string') {
          _this15.writeMessage($.i18n('api-error', pageLink + ': ' + data.error));
        } else {
          _this15.writeMessage($.i18n('api-error-unknown', pageLink));
        }
      });
    }
    ...
    value: function processTemplate(project, template, cb) {
      ...
      this.massApi(requestData, project, 'ticontinue', function (data) {
        ...
        if (data.error) {
          return _this16.apiErrorReset('Transclusion API', data.error.info);
        }
        ...
        if (data.continue) {
          _this16.writeMessage($.i18n('massviews-oversized-set-unknown', templateLink, _this16.config.apiLimit));
        }
        ...
        /** structured error comes back as a string, otherwise we don't know what happened */
        if (data && typeof data.error === 'string') {
          _this16.writeMessage($.i18n('api-error', templateLink + ': ' + data.error));
        } else {
          _this16.writeMessage($.i18n('api-error-unknown', templateLink));
        }
      });
    }
    ...
    value: function processWikiPage(project, page, cb) {
      ...
      this.massApi(requestData, project, 'plcontinue', function (data) {
        ...
        if (data.error) {
          return _this17.apiErrorReset('Links API', data.error.info);
        }

        // this happens if there are no wikilinks or the page could not be found
        if (!data.pages[0]) {
          return _this17.setState('initial', function () {
            _this17.writeMessage($.i18n('api-error-no-data'));
          });
        }
        ...
        if (!pages.length) {
          return _this17.setState('initial', function () {
            _this17.writeMessage($.i18n('massviews-empty-set', pageLink));
          });
        }
        ...
        if (data.continue) {
          _this17.writeMessage($.i18n('massviews-oversized-set-unknown', pageLink, _this17.config.apiLimit));
        }

        _this17.getPageViewsData(pages, project).done(function (pageViewsData) {
          cb(page, pageLink, pageViewsData);
        });
      }).fail(function (data) {
        var errorMessage = data && typeof data.error === 'string' ? data.error : null;
        _this17.apiErrorReset('Links API', errorMessage);
      });
    }
    ...
    value: function processQuarry(cb) {
      ...
      $.getJSON(url).done(function (data) {
        ...
        if (titleIndex === -1) {
          ...
          return _this18.writeMessage($.i18n('invalid-quarry-dataset', 'page_title'));
        }
        ...
        if (titles.length > _this18.config.apiLimit) {
          _this18.writeMessage($.i18n('massviews-oversized-set', quarryLink, _this18.formatNumber(titles.length), _this18.config.apiLimit));

          titles = titles.slice(0, _this18.config.apiLimit);
        }

        _this18.getPageViewsData(titles, project).done(function (pageViewsData) {
          cb(id, quarryLink, pageViewsData);
        });
      }).fail(function (data) {
        _this18.setState('initial');
        return _this18.writeMessage($.i18n('api-error-unknown', 'Quarry API'), true);
      });
    }
    ...
    value: function processExternalLink(cb) {
      ...
      this.massApi(requestData, project, 'euoffset', 'exturlusage').done(function (data) {
        if (data.error) {
          return _this19.apiErrorReset('External link API', data.error.info);
        }
    ...
    value: function exportCSV() {
      ...
      var csvContent = 'data:text/csv;charset=utf-8' + glamorousCols + ',Title,' + this.getDateHeadings(false).join(',') + '\n';
      ...
      this.outputData.listData.forEach(function (page) {
        ...
        glamorousCols = [];
        if (_this21.source === 'glamorous') {
          glamorousCols = ['"' + page.globalPage.descore().replace(/"/g, '""') + '"', '"' + page.project + '"'];
        }
  ...
  return function (_superclass) {
    ...
    _createClass(_class, [{
      ...
      value: function getPageviewsURL(project, page) {
        ...
        return '/pageviews?start=' + startDate.format('YYYY-MM-DD') + '&end=' + endDate.format('YYYY-MM-DD') + ('&project=' + project + '&platform=' + platform + '&pages=' + encodeURIComponent(page));
      }
...
var Pv = function (_PvConfig) {
  ...
  _createClass(Pv, [{
    ...
    value: function patchUsage() {
      ...
      var langApps = ['siteviews', 'massviews'];
