...
function drawData(data) {
  ...
  var _arr = [].concat(_toConsumableArray(Array(config.pageSize).keys()));

  for (var _i = 0; _i < _arr.length; _i++) {
    var index = _arr[_i];
    var item = session.pageData[index + offset];

    // FIXME: loop only through first 100, then add "show more" link
    // cache data
    var width = 100 * (item.views / max);
    $(".chart-container").append('<div class=\'topview-entry\' style=\'background:linear-gradient(to right, #EEE ' + width + '%, transparent ' + width + '%)\'>' + '<span class=\'topview-entry--remove glyphicon glyphicon-remove\' aria-hidden=\'true\'></span>' + ('<span class=\'topview-entry--rank\'>' + (index + offset + 1) + '</span>') + ('<a class=\'topview-entry--label\' href="https://en.wikipedia.org/wiki/' + item.article + '" target="_blank">' + item.article.replace(/_/g, ' ') + '</a>') + "<span class='topview-entry--leader'></span>" + ('<a class=\'topview-entry--views\' href=\'/pageviews#pages=' + item.article + '\'>' + pv.n(item.views) + '</a></div>'));
  }
}
...
function validateProject() {
  ...
  if (siteDomains.includes(project)) {
    ...
    resetView();
    writeMessage('<a href=\'//' + project + '\'>' + project + '</a> is not a ' + "<a href='//meta.wikipedia.org/w/api.php?action=sitematrix&formatversion=2'>valid project</a>", 'validate', true);
...
},{"../shared/pv":3,"../shared/site_map":4,"./config":7,"./session":8}],7:[function(require,module,exports){
'use strict';
...
},{"../shared/pv":3}],8:[function(require,module,exports){
'use strict';
