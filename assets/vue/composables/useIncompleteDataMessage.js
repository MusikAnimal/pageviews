 ...
 * as a quiet inline message beneath the chart while the current
 * result has a trimmed trailing date (the store's incompleteDate, set
 * by trimIncompleteTail). It clears itself as soon as a new query
 * starts.
 *
 * @param {Object} store Any app store exposing `incompleteDate`.
 * @return {{ message: import('vue').ComputedRef<string|null> }}
 */
export function useIncompleteDataMessage( store ) {
	const preferences = usePreferencesStore();

	const message = computed( () => {
		if ( !store.incompleteDate || store.status !== 'complete' ) {
			return null;
		}
		return banana.i18n(
			'api-incomplete-data',
