			...
			<header class="app-output-header">
				...
				<div class="app-output-header__heading">
					<h2 class="app-output-header__title">
						...
						<span class="app-output-header__source">({{ sourceLabel }})</span>
...
import { describeSource, getSourceItems } from '../apps/massviews/sources.js';
...
// The source's menu label, echoed next to the results title.
const sourceLabel = computed( () => getSourceItems()
	.find( ( item ) => item.value === store.source )?.label );
 ...
 * like the legacy tool (shared with the FAQ's sources section).
 */
const sourceDescription = computed( () => describeSource( store.source ) );
