	...
	<div class="app-workspace">
		...
		<figure v-else class="app-chart">
			...
			<ChartPanel
				...
				:aria-label="$i18n( 'redirectviews-title' )"
			/>
			<template v-else>
...
import RedirectviewsSettings from '../apps/redirectviews/Settings.vue';
import FaqDialog from '../apps/redirectviews/FaqDialog.vue';
...
function onDialogToggle( open ) {
	if ( !open ) {
		router.replace( { path: '/redirectviews', query: route.query } );
	}
}
...
const exportFilename = computed(
	() => `redirectviews-${ settings.start }-${ settings.end }`
);
...
function listCsvRows() {
	return [
		[ 'Title', ...store.dates ],
		...store.redirectData.map( ( row ) => [ row.title, ...row.counts ] )
	];
}

// The chart shows the combined views across the target and all its
// redirects.
